...
export default function Modal() {
    ...
    const renderIngredients = () => {
        ...
        for(let i = 1;  i<=6; i++){
            ...
            const measure = selectedRecipe[`strIngredient${i}` as keyof Recipe]

            if(ingredient && measure){
                ingredients.push(
                    <li key={i} className='text-lg'>{ingredient} - {measure}</li>
                )
            }
        }
              ...
              >
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6" >
                  <Dialog.Title as="h3" className="text-gray-900 text-4xl font-extrabold my-5 text-center">
                      ...
                      <img className='mx-auto w-96 mt-4' src={`${selectedRecipe.strDrinkThumb}`} alt="imagen bebida" />
                  </Dialog.Title>
                  ...
                  <div>
                    ...
                    >{favoriteExist(selectedRecipe.idDrink)? `Eliminar favorito` : `Anadir a favoritos`}</button>
                  </div>
                </Dialog.Panel>
              </Transition.Child>
            </div>
